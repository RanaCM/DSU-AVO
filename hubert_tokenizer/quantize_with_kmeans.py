...
import joblib
from textless_nlp.gslm.speech2unit.clustering.utils import (
    get_audio_files,
)
from textless_nlp.gslm.speech2unit.pretrained.utils import (
    get_features,
)
...
def get_parser():
    ...
    parser.add_argument(
        "--save_npy", action="store_true", help="Whether to save npy files or not"
    ) # <2.12>
    parser.add_argument(
        "--npy_path", type=str, help="Path for saving npy files"
    ) # <2.12>
    return parser
...
def main(args, logger):
    ...
    with open(args.out_quantized_file_path, "w") as fout:
        for i, feats in enumerate(features_batch):
            ...
            # breakpoint()
            base_fname = fnames[i].rstrip(args.extension) # <2.12> 
            fout.write(f"{base_fname}|{pred_str}\n")
            if args.save_npy:
                os.makedirs(os.path.join(args.npy_path, "/".join(base_fname.split("/")[:-1])), exist_ok=True)
                np.save(os.path.join(args.npy_path, base_fname), pred)
